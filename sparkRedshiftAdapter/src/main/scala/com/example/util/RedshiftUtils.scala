...
import com.example.constant.SparkConstants._
import org.apache.spark.sql.{DataFrame, SaveMode, SparkSession}

object RedshiftUtils {

  def getSparkSession(masterUrl: String,
                      appName: String,
                      awsAccessKeyId: String,
                      awsSecretAccessKey: String): SparkSession = {
    SparkSession
      .builder()
      .master(masterUrl)
      .appName(appName)
  ...
  def saveDFToRedshift(df: DataFrame,
                       table: String,
                       tempS3Dir: String): Unit = {
    df.write
      .format("com.databricks.spark.redshift")
      .option("url", redshiftJdbcURL)
